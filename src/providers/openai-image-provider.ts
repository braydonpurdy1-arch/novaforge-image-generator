...
export class OpenAiImageProvider implements ImageProvider {
  ...
  async execute(plan:ProviderExecutionPlan):Promise<ProviderResult>{
    ...
    const r=await this.options.transport({payload:{model:this.options.model,prompt,operation:plan.request.operation,references:plan.request.sourceAssets.map(a=>({id:a.id,roles:a.roles})),locks:plan.request.locks,allowedChanges:plan.request.allowedChanges,parameters:plan.parameters??{}},mediaBindings:plan.request.sourceAssets.map(a=>({id:a.id,uri:a.uri}))});
